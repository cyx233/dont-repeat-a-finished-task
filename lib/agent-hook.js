...
// Read session's original cwd from transcript file
function getSessionCwd(opts) {
  if (opts.cwd) return opts.cwd;
  if (opts.transcriptPath) {
    try {
      const chunk = fs.readFileSync(opts.transcriptPath, { encoding: 'utf8', flag: 'r' }).slice(0, 4096);
      const m = chunk.match(/"cwd":"([^"]+)"/);
      if (m) return m[1];
    } catch {}
  }
  return undefined;
