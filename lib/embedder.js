...
async function ensureDep() {
  try { await import('@huggingface/transformers'); } catch {
    const { execSync } = require('child_process');
    const pluginRoot = path.resolve(__dirname, '..');
    execSync('npm install --omit=dev', { cwd: pluginRoot, stdio: 'pipe' });
  }
}
...
async function matchByVector(prompt, catalog, opts = {}) {
  ...
  // Backfill missing vectors — guarantees correctness even if skill didn't run
  const missing = catalog.filter(item => !vectors[item.name]);
  if (missing.length) {
